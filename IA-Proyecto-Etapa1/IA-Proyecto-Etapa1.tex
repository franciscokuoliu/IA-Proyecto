...
\begin{itemize}
    \item \textbf{Keyword-Based Approaches}: Identify explicit emotional terms using lexical databases (such as WordNet-Affect or SentiWordNet) and apply linguistic rules to assess sentiment intensity \cite{7}.
    
    \item \textbf{Corpus-Based Approaches}: Induce domain-specific word-emotion lexicons using statistical methods and generative models across annotated or weakly-labeled text datasets \cite{7}.
    
    \item \textbf{Rule-Based Approaches}: Apply expert heuristic rules, Part-of-Speech tagging, and phrasal patterns to infer emotional states based on predefined linguistic structure \cite{7}.
    
    \item \textbf{Traditional Machine Learning Approaches}: Utilize supervised algorithms, such as, Support Vector Machines (SVM), Na\"ive Bayes, Decision Trees, and $K$-Nearest Neighbors—that rely on manual feature engineering to classify text into emotional categories \cite{7}.
